...
from assets.data import load_tickets, save_tickets, load_ticket_id, mark_ticket_used
                    ...
                    quart.jsonify(
                        {
                            "status": "error",
                            "message": "Ticket already used",
                            "data": load_ticket_id(ticket_id),
                        }
                    ),
                    200,
                )

            # We won the claim; record the successful access attempt. mark_ticket_used
            # already set used_at and valid=0, so only append the audit entry.
            ticket = load_ticket_id(ticket_id)
            ticket["access_attempts"].append(
                {
                    "status": "success",
                    "type": "valid",
